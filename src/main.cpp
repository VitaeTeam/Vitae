...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 VIT.
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            ...
            if (fDebug)
                LogPrintf("%s: Fundamentalnode payment check skipped on sync - skipping IsBlockPayeeValid()\n", __func__);
        }
    }
        ...
        if (strCommand == "tx") {
            ...
            if (pmn != NULL) {
                ...
                if (!CMessageSigner::VerifyMessage(pmn->pubKeyFundamentalnode, vchSig, strMessage, errorMessage)) {
                    return error("dstx: Got bad fundamentalnode address signature %s, error: %s", vin.ToString(), errorMessage);
                }

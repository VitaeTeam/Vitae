...
CzVITTracker::~CzVITTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzVITTracker::Init()
{
...
bool CzVITTracker::Archive(CMintMeta& meta)
{
...
bool CzVITTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzVITTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzVITTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzVITTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzVITTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzVITTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzVITTracker::GetSerialHashes()
{
...
CAmount CzVITTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzVITTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzVITTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzVITTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzVITTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzVITTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzVITTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzVITTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzVITTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzVITTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
{
    bool iszVITAEWalletInitialized = (NULL != zVITWallet);
    CMintMeta meta;
...
void CzVITTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzVITTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzVITTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzVITTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzVITTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzVITTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzVITWallet* zVITWallet = new CzVITWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zVITWallet);
        }
        delete zVITWallet;

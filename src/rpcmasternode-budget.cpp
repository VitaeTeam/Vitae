                       ...
                       int &nPaymentCount, int &nBlockStart, CBitcoinAddress &address, CAmount &nAmount)
{
    ...
    if (nAmount < 10 * COIN)
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Invalid amount - Payment of %d is less than minimum 10 VIT allowed", FormatMoney(nAmount)));
...
UniValue preparebudget(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw std::runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"vitae-address\" monthly-payment\n"
    ...
    CWalletTx wtx;
    if (!pwalletMain->GetBudgetSystemCollateralTX(wtx, budgetProposalBroadcast.GetHash(), useIX)) { // 50 VIT collateral for proposal
...
UniValue submitbudget(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 7)
        throw std::runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"vitae-address\" monthly-payment \"fee-tx\"\n"

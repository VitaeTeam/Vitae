...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    if (fMatureOnly) {
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zvitTracker->GetMints(true);
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zvitTracker->ListMints(true, true, true);
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount, int blockHeight)
{
    ...
    if (GetBoolArg("-vitstake", true)) {
        for (const COutput &out : vCoins) {
            ...
            if (!Params().HasStakeMinAgeOrDepth(blockHeight, GetAdjustedTime(), utxoBlock->nHeight, utxoBlock->GetBlockTime(), getStakeModifierV2SporkValue()))
                continue;
...
bool CWallet::MintableCoins()
{
    ...
    if (nBalance > 0) {
        ...
        for (const COutput& out : vCoins) {
            CBlockIndex* utxoBlock = mapBlockIndex.at(out.tx->hashBlock);
            //check for maturity (min age/depth)
            if (Params().HasStakeMinAgeOrDepth(chainHeight, time, utxoBlock->nHeight, utxoBlock->nTime, getStakeModifierV2SporkValue()))
                return true;
        }
    }
...
void CWallet::AutoZeromint()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_20_ZEROCOIN_MAINTENANCE_MODE)) return;
...
std::string CWallet::GetUniqueWalletBackupName(bool fzvitAuto) const
{
...
void CWallet::ZVitBackupWallet()
{
    ...
    if(!GetArg("-zvitbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zvitbackuppath", ""));

...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "\nMove 0.01 VITAE from timotei to akiko with a comment and funds have 6 confirmations\n" +
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zvitTracker->ListMints(true, fMatureOnly, true);
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZvitaeSpend(nAmount, fMintChange, fMinimizeChange, nSecurityLevel, vMintsSelected, address_str);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nSpend zVITAE mints to a VIT address.\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    return DoZvitaeSpend(nAmount, false, true, 100, vMintsSelected, address_str);
}


extern UniValue DoZvitaeSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, const int nSecurityLevel, vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 4 || params.size() > 5)
        throw runtime_error(
            ...
            "5. \"address\"          (string, optional) VITAE address to spend to. If not specified, spend to change add.\n"
    ...
    return DoZvitaeSpend(nAmount, false, true, 42, vMintsSelected, address_str);
}
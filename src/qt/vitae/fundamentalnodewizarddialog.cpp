...
#include "qt/vitae/forms/ui_fundamentalnodewizarddialog.h"
...
FundamentalNodeWizardDialog::FundamentalNodeWizardDialog(WalletModel *model, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::FundamentalNodeWizardDialog),
    ...
    walletModel(model)
{
    ...
    ui->lineEditName->setPlaceholderText(tr("e.g user_fundamentalnode"));
...
void FundamentalNodeWizardDialog::showEvent(QShowEvent *event)
{
    if (ui->btnNext) ui->btnNext->setFocus();
}

void FundamentalNodeWizardDialog::onNextClicked(){
    switch(pos){
        case 0:{
...
bool FundamentalNodeWizardDialog::createMN(){
    if (walletModel) {
        /**
         *
        ...
        5) use those values on the fundamentalnode.conf
         */
        ...
        if (prepareStatus.status != WalletModel::OK) {
            returnStr = tr("Prepare fundamental node failed.\n\n%1\n").arg(returnMsg);
            return false;
        }
        ...
        if (sendStatus.status == WalletModel::OK) {
            ...
            if (boost::filesystem::exists(pathBootstrap)) {
                ...
                if (!streamConfig.good()) {
                    returnStr = tr("Invalid fundamentalnode.conf file");
                    return false;
                }
                ...
                for (std::string line; std::getline(streamConfig, line); linenumber++) {
                    ...
                    if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
                        ...
                        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
                            streamConfig.close();
                            returnStr = tr("Error parsing fundamentalnode.conf file");
                            return false;
                        }
                    }
                    lineCopy += line + "\n";
                ...
                returnStr = tr("Fundamental node created!");
                return true;
            } else{
                returnStr = tr("fundamentalnode.conf file doesn't exists");
            }
        } else {
...
void FundamentalNodeWizardDialog::onBackClicked(){
    if (pos == 0) return;
    pos--;
    switch(pos){
        case 0:{
...
void FundamentalNodeWizardDialog::inform(QString text){
    if (!snackBar)
...
void FundamentalNodeWizardDialog::initBtn(std::initializer_list<QPushButton*> args){
...
FundamentalNodeWizardDialog::~FundamentalNodeWizardDialog(){

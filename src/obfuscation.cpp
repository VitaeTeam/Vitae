...
{
    if (fFundamentalNode) LogPrint(BCLog::FUNDAMENTALNODE, "CObfuscationPool::Check() - entries count %lu\n", entries.size());
    //printf("CObfuscationPool::Check() %d - %d - %d\n", state, anonTx.CountEntries(), GetTimeMillis()-lastTimeChanged);

    if (fFundamentalNode) {
        LogPrint(BCLog::FUNDAMENTALNODE, "CObfuscationPool::Check() - entries count %lu\n", entries.size());

        // If entries is full, then move on to the next phase
        if (state == POOL_STATUS_ACCEPTING_ENTRIES && (int)entries.size() >= GetMaxPoolTransactions()) {
            LogPrint(BCLog::FUNDAMENTALNODE, "CObfuscationPool::Check() -- TRYING TRANSACTION \n");
    ...
    if (state == POOL_STATUS_FINALIZE_TRANSACTION) {
        LogPrint(BCLog::FUNDAMENTALNODE, "CObfuscationPool::Check() -- FINALIZE TRANSACTIONS\n");
    ...
    if (fFundamentalNode && state == POOL_STATUS_SIGNING && SignaturesComplete()) {
        LogPrint(BCLog::FUNDAMENTALNODE, "CObfuscationPool::Check() -- SIGNING\n");
    ...
    if ((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis() - lastTimeChanged >= 10000) {
        LogPrint(BCLog::FUNDAMENTALNODE, "CObfuscationPool::Check() -- timeout, RESETTING\n");
        UnlockCoins();
        SetNull();

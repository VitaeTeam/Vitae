...
{
    CDataStream serializedCoinSpend = ZVITModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZVITSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    //Add mints to zVIT supply (mints are forever disabled after last checkpoint)
...
bool UpdateZVITSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    ...
    // Re-add spends to zVIT supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zVIT supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);

...
CzVITAETracker::CzVITAETracker(std::string strWalletFile)
{
...
CzVITAETracker::~CzVITAETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzVITAETracker::Init()
{
...
bool CzVITAETracker::Archive(CMintMeta& meta)
{
...
bool CzVITAETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzVITAETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzVITAETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzVITAETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzVITAETracker::GetSerialHashes()
{
...
CAmount CzVITAETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzVITAETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzVITAETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzVITAETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzVITAETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzVITAETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzVITAETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzVITAETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzVITAETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzVITAETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzVITAETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzVITAEWallet* zVITAEWallet)
{
    bool iszVITAEWalletInitialized = (NULL != zVITAEWallet);
    CMintMeta meta;
    ...
    if (! iszVITAEWalletInitialized)
        zVITAEWallet = new CzVITAEWallet(strWalletFile);
    meta.isSeedCorrect = zVITAEWallet->CheckSeed(dMint);
    if (! iszVITAEWalletInitialized)
        delete zVITAEWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzVITAETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzVITAETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzVITAETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzVITAETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzVITAETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzVITAETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzVITAEWallet* zVITAEWallet = new CzVITAEWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zVITAEWallet);
        }
        delete zVITAEWallet;
